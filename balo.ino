...
void readGPS() {
  ...
  const unsigned long interval = 10000; // Đọc GPS mỗi giây
...
void readSensorData() {
  ...
  //if ((abs(roll) >= 20) || (pitch) <= -35 || (pitch) >= 9 || abs(weight / 100000) >= 3 || button == 1) {
  
  if (  ( (roll) <= Xduoi || (roll) >= Xtren || (pitch) <= Yduoi || (pitch) >= Ytren || abs(weight / 100000) >= cannang || button == 1) && (checkcoi == 1)) {
    if (!coiDangKeu) {
      coiOn();
      giatricu = millis();
      coiDangKeu = true;
...
void setup(void) {
  ...
  //setup giá trị ban đầu
  Blynk.virtualWrite(V6, -20);
  Blynk.virtualWrite(V7, 20);
  Blynk.virtualWrite(V8, -35);
